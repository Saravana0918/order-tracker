...
app.get('/api/orders-summary-metrics', async (req, res) => {
    try {
        ...
        // Aggregate order count and total quantity from order_progress
        const [agg] = await pool.query(`
            SELECT
                DATE_FORMAT(CONVERT_TZ(created_at, '+00:00', '+05:30'), '%Y-%m-%d') AS date,
        ...
        res.json(rows.reverse()); // show today first
    } catch (err) {
